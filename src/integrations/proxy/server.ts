...
import { parseToolUseBlocks, extractTokenUsage, getAllFiles, getAllFolders } from './agents/claude/parser.js';
import type { AnthropicResponse } from './agents/claude/parser.js';
import {
  createSessionState,
  getSessionState,
  updateSessionState,
...
async function postProcessResponse(
  ...
  if (isEndTurn && extendedCacheData) {
    ...
    const isUpdate = extendedCache.has(cacheKey);
